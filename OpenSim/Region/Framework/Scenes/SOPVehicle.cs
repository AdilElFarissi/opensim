...
namespace OpenSim.Region.Framework.Scenes
{
    public class SOPVehicle
    {
        ...
        public static SOPVehicle FromXml2(XmlReader reader)
        {
            ...
            // We expect the reader to be positioned at the Vehicle element.
            if (reader.NodeType != XmlNodeType.Element || reader.Name != "Vehicle")
            {
                // If not, we return null because we cannot process.
                return null;
            }

            // Read the outer XML of the current element
            string xmlContent = reader.ReadOuterXml();

            // Now create a safe reader from the string
            UTF8Encoding enc = new UTF8Encoding();
            MemoryStream ms = new MemoryStream(enc.GetBytes(xmlContent));
            XmlTextReader safeReader = new XmlTextReader(ms);
            safeReader.DtdProcessing = DtdProcessing.Ignore;

            SOPVehicle vehicle = new SOPVehicle();
            bool errors = false;

            vehicle.FromXml2(safeReader, out errors);
            safeReader.Close();
