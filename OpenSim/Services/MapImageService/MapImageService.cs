...
#pragma warning restore 414
        ...
        private static readonly Color m_Watercolor = Color.FromArgb(29, 72, 96);
        ...
        private bool UpdateMultiResolutionFiles(int x, int y, UUID scopeID)
        {
            ...
            {
                m_MultiRezToBuild.Enqueue(new MapToMultiRez { x = x, y = y, scopeID = scopeID });
        ...
        public byte[] GetMapTile(string fileName, UUID scopeID, out string format)
        {
            ...
            fullName = Path.Combine(fullName, safeFileName);
            try
            {
                lock (m_Sync)
                {
                    format = Path.GetExtension(safeFileName).ToLowerInvariant();
        ...
        {
            return Path.Combine(path.ToString(), $"map-{zoomLevel}-{x}-{y}-objects.jpg");
        }
